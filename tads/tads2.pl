...
#Game Contents
my @Objects 		= ();	# Hash-map of the objects in the game.
...
sub preloadMapping(){
	# Constant names for properties and builtins are taken from detads by Daniel Schepler
...
sub generateMapping(){
	...
	for (my $action=0 ; $action<$#Translate_Action ; $action++){
		...
		print $File_Mapping nameAaction($action) if defined $Translate_Action[$action];
		print $File_Mapping "'\n";
	}
